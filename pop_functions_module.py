...
def mortalite(donnees_dens, path_mortalite_hf):
    ...
    # Step 1: Bin ages 99+ into age 99, then aggregate mortality by age
    donnees_mortalite_hf['age'] = pd.to_numeric(donnees_mortalite_hf['age'], errors="coerce")
    donnees_mortalite_hf.loc[donnees_mortalite_hf['age'] > 98, 'age'] = 99

    mortality_cols = [col for col in donnees_mortalite_hf.columns if col != 'age']
    donnees_mortalite_hf[mortality_cols] = donnees_mortalite_hf[mortality_cols].apply(pd.to_numeric, errors="coerce")
    donnees_mortalite_hf = donnees_mortalite_hf.groupby('age', as_index=False).sum()

    # Step 2: Merge mortality data with population density data
    donnees_dens['age'] = donnees_dens['age'].astype(str)
    donnees_mortalite_hf['age'] = donnees_mortalite_hf['age'].astype(int).astype(str)
...
def export_pollution(donnees_filtrees, donnees_geom):
    import pandas as pd
    ...
    # Prepare indicator (geographic) columns
    donnees_indic = (
        donnees_geom.loc[:, ["iris_cod", "iris_name", "com_cod", "com_name", "dep_cod", "dep_name", "region_cod", "region_name"]]
        .rename(columns={
            "region_cod": "regcod",
            "region_name": "regname",
            "iris_cod": "iriscod",
    ...
    # Ensure 'iriscod' exists in donnees_filtrees for grouping and merging
    if "iriscod" not in donnees_filtrees.columns:
        donnees_filtrees["iriscod"] = donnees_filtrees["iris_cod"]

    # Filter by age and group by 'iriscod', then aggregate population and mortality data
    donnees_pop = (
        donnees_filtrees[donnees_filtrees["age"] >= 1]
...
## Function to plot gender-based population pyramids for multiple years side-by-side
def pyramide_population_genre_3years(
        path_femmes,
        path_hommes,
    ...
    def _read_population_csv(p):
        df = pd.read_csv(p)
        df.columns = df.columns.astype(str).str.strip()

        # If parsing failed due to wrong delimiter (all data in a single column), retry with common delimiters
        if df.shape[1] == 1:
        ...
        # Fallback: try delimiter inference
        if df.shape[1] == 1:
    ...
    df_f = _read_population_csv(path_femmes)
    df_h = _read_population_csv(path_hommes)

    def _normalize_age_col(df):
    ...
    df_f = _normalize_age_col(df_f)
    df_h = _normalize_age_col(df_h)

    def _find_year_col(df, year):
        # population columns are directly by year (e.g., 2019, 2030, 2050) with no substitutes
        y_str = str(year)
        if y_str in df.columns:
            return y_str
        if year in df.columns:
            return year
        raise KeyError(
            f"Could not find a population column named exactly '{y_str}' for year={year}. "
            f"Available columns: {list(df.columns)}"
        )

    cols_f = {y: _find_year_col(df_f, y) for y in years}
    cols_h = {y: _find_year_col(df_h, y) for y in years}

    ages = sorted(set(df_f["age"].unique()).union(set(df_h["age"].unique())))
    tick_ages = [a for a in ages if (a % y_tick_step) == 0]
    if ages and ages[-1] not in tick_ages:
        tick_ages.append(ages[-1])
    ...
    for y in years:
        col_f = cols_f[y]
        col_h = cols_h[y]
        sf = pd.to_numeric(df_f[col_f], errors="coerce").groupby(df_f["age"]).sum(min_count=1)
        sh = pd.to_numeric(df_h[col_h], errors="coerce").groupby(df_h["age"]).sum(min_count=1)
        series_f[y] = sf.reindex(ages, fill_value=0.0).astype(float)
        series_h[y] = sh.reindex(ages, fill_value=0.0).astype(float)

    max_val = 0.0
    for y in years:
        max_val = max(max_val, float(series_f[y].max()), float(series_h[y].max()))
    x_lim = max_val * 1.15 if max_val > 0 else 1.0
    ...
    for ax, y in zip(axes, years):
        ...
        ax.set_title(str(y))
        ax.set_yticks(tick_ages)
        ax.set_yticklabels([str(a) for a in tick_ages])
    ...
    fig.suptitle("Population Pyramid by Gender", y=1.08)
    fig.tight_layout()
    plt.show()
...
#Distribute population and mortality data by commune using Departmental population
def disaggregate_commune_by_age(commune_df, perc_hf, year=2019, age_min=None):
    frac_col = f"FR_{year}_agesurtrage"
    if frac_col not in perc_hf.columns:
        raise ValueError(f"{frac_col} not found in perc_hf columns.")
    perc_hf = perc_hf[['age', frac_col]].copy()
    perc_hf['age'] = pd.to_numeric(perc_hf['age'], errors='coerce')
    if age_min is not None:
        perc_hf = perc_hf[perc_hf['age'] >= age_min]
        perc_hf[frac_col] /= perc_hf[frac_col].sum()  # normalize
    commune_df['key'] = 1
    perc_hf['key'] = 1
    merged = commune_df.merge(perc_hf, on='key').drop('key', axis=1)
    merged['pop_age'] = merged['pop30p'] * merged[frac_col]
    merged['morta_age'] = merged['morta_a30_moy'] * merged[frac_col]
    result = merged[['comcod', 'age', 'pop_age', 'morta_age']].copy()
    result = result[result['age'].notnull()]
    result['age'] = result['age'].astype(int)
    return result

#Distribute population and mortality data Homogenously across commune for comparison
def desagreg_commune_simple(commune_df, age_list=None):
    """
    Disaggregate commune-level mortality and population homogeneously across ages 30–99.
    Parameters:
        commune_df: DataFrame with ['comcod','pop30p','morta_a30_moy']
        age_list: list of ages to distribute (default 30–99)
    Returns:
        DataFrame with ['comcod','age','pop_age','morta_age']
    """
    if age_list is None:
        age_list = list(range(30, 100))

    df_list = []
    for _, row in commune_df.iterrows():
        n_ages = len(age_list)
        age_frac = 1 / n_ages  # equal share per age
        df_tmp = pd.DataFrame({
            "comcod": row["comcod"],
            "age": age_list,
            "pop_age": row["pop30p"] * age_frac,
            "morta_age": row["morta_a30_moy"] * age_frac
        })
